...
def combine_diag_proc_codes(hadm_id_set, out_filename='ALL_CODES_filtered.csv'):
    ...
    # filter the code which has related patient
    ...
    #output the ALL_CODES_filtered.csv
...
def combine_notes_codes(disch_full_filename, filtered_codes_filename, out_filename='notes_labeled.csv'):
    ...
    #read the disch_full.csv then join the texts groupby hadm_id
    ...
    #read the ALL_CODES_filtered.csv then join the icd9_code groupby hadm_id
    ...
    #merge the dischfull and allcodesfiltered
...
def split_data(labeled_notes_filename='notes_labeled.csv', is_full=True):
    ...
    #unzip the counter, back to 2-dimension matrix.
    ...
    if is_full:
        ...
        for split in ['train', 'dev', 'test']:
            ...
            print("isin full: ")
            print(labeled_notes_df['HADM_ID'].isin(split_hadm_ids))
        ...
        # Divide test, dev, train
        all_labeled_notes_df = pd.read_csv(f'{constants.GENERATED_DIR}/ALL_CODES_filtered.csv')
        all_hadm_with_top_disease_df = all_labeled_notes_df[all_labeled_notes_df['ICD9_CODE'].isin(top_codes)]
        all_hadm_with_top_disease_size = len(all_hadm_with_top_disease_df['HADM_ID'].unique())
        train_size = int(all_hadm_with_top_disease_size * 0.6)
        test_size = int(all_hadm_with_top_disease_size * 0.2)
        dev_size = int(all_hadm_with_top_disease_size * 0.2)
        all_hadm_with_top_disease = all_hadm_with_top_disease_df['HADM_ID'].unique()
        train_hadm_with_top_disease = all_hadm_with_top_disease[:train_size]
        test_hadm_with_top_disease = all_hadm_with_top_disease[train_size:train_size + test_size]
        dev_hadm_with_top_disease = all_hadm_with_top_disease[train_size + test_size:]
        pd.DataFrame(train_hadm_with_top_disease).to_csv(f'{constants.CAML_DIR}/train_50_hadm_ids.csv')
        pd.DataFrame(test_hadm_with_top_disease).to_csv(f'{constants.CAML_DIR}/test_50_hadm_ids.csv')
        pd.DataFrame(dev_hadm_with_top_disease).to_csv(f'{constants.CAML_DIR}/dev_50_hadm_ids.csv')
        for split in ['train', 'dev', 'test']:
            split_hadm_id_df = pd.read_csv(f'{constants.CAML_DIR}/{split}_{content}_hadm_ids.csv', names=['HADM_ID'])
            split_hadm_ids = split_hadm_id_df['HADM_ID'].values.tolist()
            #split_labeled_notes_df = labeled_notes_df[labeled_notes_df['HADM_ID'].isin(split_hadm_ids)]
